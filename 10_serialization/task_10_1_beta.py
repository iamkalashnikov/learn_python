...
def parse_sh_version(output):
	with open(output) as data:
		for i in result_vers:
			
			for line in data:
				match = re.search(regex, line)
				if match and result_parse:
					result_parse.append('{:<10}'.format(match.group().rstrip().replace('"', '')))
				elif match:
					result_parse.append('{:<10}'.format(i))
					result_parse.append('{:<10}'.format(match.group().rstrip().replace('"', '')))
	return tuple(result_parse)

l_to_f = [headers, parse_sh_version('sh_version_r1.txt')]
